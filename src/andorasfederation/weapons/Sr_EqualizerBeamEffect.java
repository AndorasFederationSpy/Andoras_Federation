...
public class Sr_EqualizerBeamEffect implements BeamEffectPlugin {
	...
	//beam particles
	private static final Color PARTICLE_COLOR = new Color(0, 199, 33, 255);
    private static final float PARTICLE_SIZE_MIN = 7f;
    private static final float PARTICLE_SIZE_MAX = 11f;
    private static final float PARTICLE_DURATION_MIN = 0.3f;
    private static final float PARTICLE_DURATION_MAX = 0.7f;
    ...
    private static final float PARTICLE_DENSITY = 0.25f; //Measured in particles per SU^2 (area unit) and second; lower means less particles. This is multiplied by charge level, too
    private static final float PARTICLE_SPAWN_WIDTH_MULT = 0.14f; //Multiplier for how wide the particles are allowed to spawn from the beam center; at 1f, it's equal to beam width, at 0f they only spawn in the center. Note that true beam width is usually much bigger than the visual beam width
	...
	public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) 
	{
		...
		if (target instanceof CombatEntityAPI) 
		{
			...
			if(weapon.isFiring() || (weapon.getChargeLevel() > 0f))
			{
				...
				Color color = new Color(80, 199, 154, 255);
                ...
                velocity = MathUtils.getRandomPointInCircle(velocity, PARTICLE_DRIFT);
					...
					if((float) Math.random() <= 0.05f)
					{
						MagicLensFlare.createSharpFlare(
							...
							new Color (11,150,19,150),
							new Color(99,255,190,255)
						);
					}
